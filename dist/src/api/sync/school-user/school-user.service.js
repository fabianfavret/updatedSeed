...
let SchoolUsersService = SchoolUsersService_1 = class SchoolUsersService extends defatul_service_1.DefaultService {
    ...
    async _create(params) {
        try {
            this.logger.debug(`_create: ${params.users.guid}`);
            await this.tangerineService.post({ url: tangerine_service_1.TANGERINE_URL.POST_SCHOOL_USER, path: { guid: params.schoolGuid }, payload: params.users });
        }
        catch (error) {
    ...
    async sync(params) {
        ...
        try {
            ...
            if (!guids.length) {
                return this.logger.log(`Discard add Users to School`);
            }
            await this._create({ users: { guid: guids }, schoolGuid: params.schoolGuid });
        }
        catch (error) {
...
//# sourceMappingURL=school-user.service.js.map